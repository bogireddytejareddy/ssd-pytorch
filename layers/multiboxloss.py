...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, preds, targets):
        ...
        loss_l = F.smooth_l1_loss(loc_preds[non_background_mask, :], loc_tars[non_background_mask, :],
                                  size_average=False)

        loss_gt = -torch.log(F.softmax(conf_preds.view(-1, self.num_classes)))
        loss_gt = loss_gt.gather(dim=1, index=class_tars.view(-1,
                                                              1))  # find softmax of class from bt_sz * priors * num_classes
        loss_gt[non_background_mask.view(-1, 1)] = 0  # zero non background
        loss_gt = loss_gt.view(batch_size, -1)

        _, loss_sort_idx = loss_gt.sort(1,
                                        descending=True)  # find backgrounds with highest softmax value 300, 500, 200, 100, 600
        _, idx_rank = loss_sort_idx.sort(1)  # sort 100, 200, 300, 500, 600 -> 4, 3, 1, 2, 5

        num_neg_each_sample = torch.clamp(3 * num_pos_each_sample,
                                          max=non_background_mask.size(1) - 1)  # fix no. of neg samples
        background_mask = idx_rank < num_neg_each_sample.expand_as(
            idx_rank)  # if neg_samples > actual rank from sort of softmax then consider
